...
typedef struct RunaheadCheckpoint {
  ...
  uint32_t checkpoint_id; // checkpoint_id generated by hardware
  uint32_t pc; // branch / load inst's pc
  // bool branch;
  bool may_replay;
} RunaheadCheckpoint;
...
typedef struct MemdepQueryResponse {
  ...
  // uint32_t mem_access_paddr;
  uint32_t mem_access_vaddr;
  // uint32_t mem_access_result;
  bool ref_need_wait;
} MemdepQueryResponse;
...
public:
  ...
  Runahead(int coreid);
  ~Runahead();
  void register_checkpoint(pid_t pid, uint32_t branch_checkpoint_id, uint32_t branch_pc, bool may_replay);
  pid_t free_checkpoint();
  void recover_checkpoint(uint32_t checkpoint_id);
  void restart();
  void update_debug_info(void* dest_buffer);
  int step();
  bool checkpoint_num_exceed_limit();
  int do_instr_runahead();
  pid_t do_instr_runahead_pc_guided(uint32_t jump_target_pc);
  pid_t init_runahead_slave();
  pid_t fork_runahead_slave();
  void runahead_slave();
  pid_t request_slave_runahead();
  pid_t request_slave_runahead_pc_guided(uint32_t target_pc);
